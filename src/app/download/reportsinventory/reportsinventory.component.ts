...
@Component({
  selector: 'app-reportsinventory',
  templateUrl: './reportsinventory.component.html',
  styleUrls: ['./reportsinventory.component.css']
})
export class ReportsinventoryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._spinner.show();
    this._DownloadService.getDownloadScreens('Inventory')
      .subscribe(
        (data: DownloadMaster[]) => {
  ...
  Downloadexcel(): void {
    ...
    if (this._authorizationGuard.CheckAcess("Reportsinventory", "ViewEdit")) {
      return;
    }
    ...
    // 91	-MIS
    // 92	-Others 
      const MenuId = 87; //87|88|89|90|91|92; 
      this._spinner.show(),
