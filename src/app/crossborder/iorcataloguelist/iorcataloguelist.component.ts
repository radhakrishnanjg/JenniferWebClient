...
@Component({
  selector: 'app-iorcataloguelist',
  templateUrl: './iorcataloguelist.component.html',
  styleUrls: ['./iorcataloguelist.component.css']
})
export class IORCataloguelistComponent implements OnInit {
  ...
  onLoad(SearchBy: string, Search: string, ) {
    ...
    return this._catalogueService.IORSearch(SearchBy, Search, startdate, enddate).subscribe(

      (lst) => {
        if (lst != null) {
          this.items = lst;
          this.UnderConsultant('ResentToMerchant');
          this.loadItems();
  ...
  UnderConsultant(Status: string) {
    this.items1 = this.items.filter(a => a.IOR_CatalogueStatus != Status);
    this.loadItems();
  }

  ResenttoMerchant(Status: string) {
    this.items1 = this.items.filter(a => a.IOR_CatalogueStatus == Status);
    this.loadItems();
  }
