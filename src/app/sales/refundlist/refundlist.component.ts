...
import { Refund } from '../../_services/model';
...
export class RefundlistComponent implements OnInit { 
  ...
  confirmDeleteid(id: number, DeleteColumnvalue: string) {
    if (this._authorizationGuard.CheckAcess("Refundlist", "ViewEdit")) {
      return;
    }
  ...
  delete() { 
    this._creditnoteService.RefundDelete(this.selectedDeleteId).subscribe(
      (data) => {
  ...
  onLoad(SearchBy: string, Search: string, ) {
    ...
    return this._creditnoteService.RefundSearch(SearchBy, Search, startdate, enddate).subscribe(
      (lst) => {
        if (lst != null) { 
          this.items = lst;
          this.loadItems();
